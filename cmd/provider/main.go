...
import (
	...
	"github.com/terasky-oss/provider-aerospike/apis"
	"github.com/terasky-oss/provider-aerospike/apis/v1alpha1"
	"github.com/terasky-oss/provider-aerospike/config"
	"github.com/terasky-oss/provider-aerospike/internal/clients"
	"github.com/terasky-oss/provider-aerospike/internal/controller"
	"github.com/terasky-oss/provider-aerospike/internal/features"
)

func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for AeroSpike").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-aerospike"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-aerospike",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add AeroSpike APIs to scheme")
	...
	kingpin.FatalIfError(controller.Setup(mgr, o), "Cannot setup AeroSpike controllers")
